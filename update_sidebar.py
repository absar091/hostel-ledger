...
nav_replace = '''            <Tooltip key={item.id}>
...
nav_end_search = '''              {item.badge > 0 && (
                <span className={cn(
                  "absolute bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white",
                  isOpen ? "right-2 top-1/2 -translate-y-1/2" : "right-1 top-1"
                )}>
