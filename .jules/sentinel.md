...
## 2024-05-29 - Privilege Escalation via Cascading Write Rules
**Vulnerability:** The Firebase Realtime Database rules used `".write": false` on child fields (like `walletBalance` and `settlements`) to prevent users from modifying them. However, since the parent node (`users/$uid`) allowed writes (`".write": "$uid === auth.uid"`), the child write rules were overridden because `.write` rules cascade in Firebase.
**Learning:** `.write` and `.read` rules in Firebase cascade and cannot be revoked by more restrictive child rules. To protect specific fields while allowing writes to the parent object, `.validate` rules must be used, as they do not cascade.
**Prevention:** Use `".validate": false` on individual fields that should not be modified by clients, rather than relying on `".write": false` on child nodes.
