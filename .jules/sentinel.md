...
## 2024-06-12 - Prevent Privilege Escalation in Firebase Rules
**Vulnerability:** The `/users/$uid` node allows users to write to their own profile, but lacked validation rules to restrict modifications to the `role` and `accountStatus` fields. This means a user could theoretically elevate their own privileges by manually setting their `role` to `admin` or reinstate a banned account by setting their `accountStatus` back to `active` via the Firebase Realtime Database SDK.
**Learning:** In Firebase Realtime Database, parent `.write` rules cascade and override child `.write` rules. Therefore, a user with `.write` access to a parent node can bypass child `.write` restrictions by updating the entire parent object.
**Prevention:** To securely restrict specific child fields (like `role` or `accountStatus`), you must use `.validate` rules, which cascade downwards and cannot be bypassed by parent `.write` permissions.
