...
## $(date +%Y-%m-%d) - [Fix Firebase Rules Privilege Escalation]
**Vulnerability:** Users could escalate their privileges to admin/superadmin or tamper with their wallet balance by sending targeted child writes (e.g., `users/$uid/role`).
**Learning:** In Firebase Realtime Database, `.write` rules cascade and cannot be revoked by child rules (making `".write": false` ineffective if a parent grants write access). Furthermore, parent `.validate` rules do NOT cascade to targeted child writes.
**Prevention:** To securely protect a child field (like `role` or `walletBalance`), its `.validate` rule must be explicitly defined directly on that exact child node (e.g., `".validate": "false"`).
