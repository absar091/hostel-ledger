...
**Vulnerability:** Found `<a>` tags with `target="_blank"` missing the `rel="noopener noreferrer"` attribute.
**Learning:** React Router `<Link>` tags with `target="_blank"` might automatically imply `noopener noreferrer` depending on the version/configuration, but standard HTML `<a>` tags do not. When internal links (e.g. `/terms-of-service`) are rendered as `<a>` tags instead of `<Link>` components, they are vulnerable to reverse tabnabbing where the newly opened window could potentially navigate the original window to a malicious URL if it's somehow redirected.
**Prevention:** Always add `rel="noopener noreferrer"` explicitly to any `<a>` tag that includes `target="_blank"`, regardless of whether it points to an internal or external URL.
