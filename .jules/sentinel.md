...
## 2026-03-18 - [CRITICAL] DoS via Object Injection and Open Relay via Email Override
**Vulnerability:** Express endpoints lacked `typeof` string validation allowing object injection, and `/support` allowed overriding email for open relay attacks.
**Learning:** Always validate input type as string before using it, and restrict email parameters to authenticated user state.
**Prevention:** Implement strict input type checking (e.g. `typeof === "string"`) and enforce `req.user.email` over `req.body.email` for system emails.
