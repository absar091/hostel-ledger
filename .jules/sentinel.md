...
## 2026-03-18 - Open Relay and NoSQL Injection in Support Route
**Vulnerability:** The `/support` endpoint in `userRoutes.js` accepted an `email` field from `req.body` directly without validating it against `req.user.email`. This allowed potential open relay attacks where a user could submit support tickets on behalf of others. Additionally, `req.body.subject` and `req.body.message` lacked string type validation, potentially leading to NoSQL injection or DoS.
**Learning:** Even authenticated endpoints that handle support requests must rigorously validate input types to prevent NoSQL injection and ensure that any user-provided email strictly matches the authenticated user's email to prevent spoofing or open relay abuse.
**Prevention:** Always enforce `typeof req.body.field === 'string'` for all string inputs, and strictly compare `req.body.email` with `req.user.email` for endpoints that trigger emails or create records on behalf of a user.
