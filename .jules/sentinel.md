...
## 2024-06-25 - Open Relay Vulnerability in Support Ticket Endpoint
**Vulnerability:** The `/support` endpoint in `userRoutes.js` accepted an `email` parameter from the request body and used it directly in `ticketData` which triggered a confirmation email. This allowed an authenticated user to send arbitrary support confirmation emails to any address (Open Relay).
**Learning:** Endpoints that trigger email sending, even if authenticated, must strictly validate that the recipient matches the authenticated user's email to prevent abuse and spam. In this architecture, failing to cross-check `req.body.email` with `req.user?.email` leads to a privilege escalation / abuse vector.
**Prevention:** Always verify `req.body.email === req.user?.email` or explicitly just use `req.user.email` for self-targeted notifications.
