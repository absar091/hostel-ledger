...
## 2025-02-27 - Unauthenticated Push Notification Endpoints
**Vulnerability:** The `/api/push-notify` and `/api/push-notify-multiple` endpoints in `backend-server/server.js` lacked authentication middleware, allowing any unauthenticated user to trigger push notifications to arbitrary users.
**Learning:** Endpoints that trigger external actions or communications (like emails or push notifications) are often overlooked during security reviews if they don't directly modify the database. They must always require authentication to prevent spam or phishing vectors.
**Prevention:** Always verify that all routes defined in Express apps have the appropriate authentication/authorization middleware (`authenticate`, `adminAuth`, etc.) unless they are explicitly intended to be public (like login/signup).
