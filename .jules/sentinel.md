...
## 2024-05-28 - Open Relay Vulnerability in Support Endpoint
**Vulnerability:** The `/api/support` endpoint used the `email` address provided in `req.body` directly in the `emailService.sendEmail` to field, allowing an attacker to submit support tickets with arbitrary email addresses and send spam/phishing emails (Open Relay) via the server's transactional email service.
**Learning:** Even on authenticated routes, user-provided metadata like `req.body.email` must never be used directly in outgoing email actions when an authenticated identity (`req.user.email`) is available.
**Prevention:** Always use the verified `req.user.email` from the authentication context for transactional notifications triggered by user actions, rather than trusting the client payload.
