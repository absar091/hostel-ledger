...
## 2024-05-18 - Missing group deletion transaction cascade cleanup
**Vulnerability:** When a group was deleted via the `/api/delete-group` user endpoint, associated transactions in the `transactions/${groupId}` node were orphaned and left in the database.
**Learning:** Orphaned records in the Firebase Realtime Database lead to unnecessary data retention, which is a privacy/security best practice violation. The admin `deleteGroupForce` method was already doing this correctly.
**Prevention:** Ensure cascade deletion operations across all endpoints performing soft/hard deletes always account for associated sub-collections or foreign keys, ideally via structured delete operations or security rules, particularly for user-managed data deletion.
