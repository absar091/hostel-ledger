...
## 2026-02-05 - Fix Privilege Escalation in Firebase Rules
**Vulnerability:** Firebase Realtime Database rules allowed privilege escalation and financial manipulation because `.write` restrictions on child nodes (`walletBalance`, `settlements`) were overridden by a permissive parent `.write` rule on the `/users/$uid` node.
**Learning:** In Firebase Realtime Database, parent `.write` rules cascade and override any child `.write` restrictions. To effectively secure child nodes when a parent node is writable, `.validate` rules must be used, as they cannot be bypassed.
**Prevention:** Always use `.validate` rules (e.g., `".validate": "false"`) instead of `.write` rules to restrict modification of sensitive fields when users have write access to the parent object.
