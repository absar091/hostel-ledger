...
## 2025-02-09 - Missing Owner Verification on Node Creation
**Vulnerability:** The Realtime Database rules allowed any authenticated user to create groups on behalf of other users because the `newData.child('createdBy').val()` was not validated against `auth.uid` during node creation.
**Learning:** Even if `.write` rules restrict updates to the owner (`data.child('createdBy').val() === auth.uid`), node creation (`!data.exists()`) via `.write` must be paired with `.validate` checks to ensure the newly created ownership field matches the requestor's UID.
**Prevention:** Always pair `!data.exists()` write conditions with `.validate` rules that explicitly enforce `newData.child('ownerField').val() === auth.uid` to prevent impersonation during creation.
