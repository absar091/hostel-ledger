...
## 2024-06-25 - Privilege Escalation via Cascading Firebase Rules
**Vulnerability:** The Firebase Realtime Database rules for the `users/$uid` node granted users parent `.write` access (`$uid === auth.uid`). It attempted to restrict writes to sensitive fields like `walletBalance` and `settlements` using child `.write: false` rules. However, Firebase parent `.write` rules cascade and override child `.write` restrictions, allowing clients to bypass them by replacing the entire user object. There were also no validation rules preventing users from adding a `role: 'admin'` field.
**Learning:** In Firebase Realtime Database, parent `.write` permissions cannot be revoked at the child level using `.write: false`. A user with write access to a parent node can overwrite any child nodes.
**Prevention:** To securely restrict specific child fields (like `role`, `accountStatus`, or `walletBalance`) when the user has parent `.write` access, you must use `.validate` rules on those specific child fields. `.validate` rules are evaluated independently of `.write` rules and cannot be bypassed by parent permissions.
