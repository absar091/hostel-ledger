...
## 2024-05-24 - Missing input validation on targetId
**Vulnerability:** The `/api/user/report` endpoint accepted a `targetId` directly from the request body without validation and stored it in the database.
...
**Prevention:** Always explicitly validate client-provided IDs using `isValidFirebaseId` before saving them to prevent maliciously formatted payloads from being stored or used in secondary contexts.
