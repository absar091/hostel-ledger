...
## 2024-05-25 - IDOR in AI Expense Endpoints
**Vulnerability:** The `/api/ai/parse-expense` and `/api/ai/parse-expense-audio` endpoints did not verify if the authenticated user (`req.user.uid`) actually belonged to the specified group (by checking `userGroups/${req.user.uid}/${groupId}`) before accessing group details to act as context for AI parsing, exposing the endpoints to Insecure Direct Object Reference (IDOR).
**Learning:** Endpoints that handle group-scoped data must always explicitly verify the requesting user's authorization to access the referenced object, even if the primary purpose of the endpoint is to offload work to a third-party AI.
**Prevention:** Always verify user membership against `userGroups/${req.user.uid}/${groupId}` prior to querying group-scoped data.
