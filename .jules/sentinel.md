...
## 2026-08-26 - Express Route Ordering Vulnerability
**Vulnerability:** Rate limiters were completely bypassed for critical endpoints.
**Learning:** In Express, middleware executes in the exact order it is defined. If a route is mounted before a middleware like `express-rate-limit`, the rate limiting logic will never execute for requests matching that route.
**Prevention:** Always mount global security middlewares (like `express-rate-limit`) strictly before any routers they are intended to protect.
