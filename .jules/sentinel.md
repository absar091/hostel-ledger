...
## 2026-09-06 - Open Relay Vulnerability in Support Ticket Endpoint
**Vulnerability:** The `/support` endpoint in `backend-server/routes/userRoutes.js` allowed users to provide an arbitrary `email` parameter in the request body, which was then used as the recipient for the support ticket confirmation email.
**Learning:** Endpoints that trigger emails must strictly ensure they only send emails to the authenticated user's verified email address (e.g., `req.user.email`) unless explicitly designed for inviting external users (and properly rate-limited).
**Prevention:** Always use `req.user.email` for user-targeted emails and do not trust email addresses provided in the request body.
