...
## 2024-05-24 - Prevent Open Relay on Support Tickets
**Vulnerability:** User-provided email in `req.body` used for support ticket emails.
**Learning:** Automated email services should not trust user input when sending confirmation emails if authenticated email is available.
**Prevention:** Use `req.user.email` from validated token context.
