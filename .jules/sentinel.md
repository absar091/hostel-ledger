...
## 2024-10-24 - Privilege escalation via cascading write rules in Firebase RTDB
**Vulnerability:** Users had `.write` access to their entire `users/$uid` node. Because Realtime Database write rules cascade, users could overwrite their `role` (to 'admin') or `accountStatus` by passing those fields in an update object.
**Learning:** Parent `.write` rules cascade and override child `.write` rules. To securely restrict specific child fields inside a parent node where the user has `.write` access, you must use `.validate` rules on those specific child fields, which cannot be bypassed.
**Prevention:** Always use `.validate` rules to enforce schema limits, defaults, and prevent modification of sensitive properties (like `role` or `status`) on nodes where users have parent `.write` permissions.
