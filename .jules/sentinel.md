...
## 2026-07-16 - 🛡️ Sentinel: [CRITICAL] Fix Firebase Rules Privilege Escalation
**Vulnerability:** Firebase Realtime Database rules cascade top-down. The parent node `/users/$uid` had `.write: "$uid === auth.uid"`, which allowed users to write to any child node, including sensitive fields like `walletBalance`, `settlements`, and `role`, even though they had `.write: false` configured.
**Learning:** Firebase Realtime Database `.write` rules cascade and cannot be overridden by child nodes. To restrict write access to specific child fields when the parent node grants write access, you must use `.validate` rules, as they do not cascade and must all evaluate to true.
**Prevention:** Always use `.validate` rules to prevent write-once privilege escalations or protect sensitive child fields when the parent node grants write access. Ensure the validation correctly handles the initial creation value (e.g., `(!data.exists() && newData.val() === 'user') || newData.val() === data.val()`) rather than simply `!data.exists()`.
