...
## 2026-08-26 - Rate Limiter Middleware Bypass
**Vulnerability:** Express routers for admin, user, and export endpoints were mounted before the global `generalLimiter` middleware in `server.js`.
**Learning:** In Express.js, middleware is executed sequentially. If a request matches a route mounted before a rate limiter, the route handler executes and the rate limiter is bypassed, leading to DoS vulnerabilities on critical endpoints.
**Prevention:** Always mount global rate-limiting and security middleware strictly before mounting the application's routing logic in Express.js.
