...
## 2024-08-23 - DoS Vulnerability via Type Juggling
**Vulnerability:** Node.js Express endpoints crashed (Denial of Service) when JSON payloads included objects (e.g., `{"$ne": null}`) instead of expected strings for fields like `username` or `email`, causing a `TypeError` when `.toLowerCase()` was called.
**Learning:** Checking for truthiness (e.g., `if (!email)`) is insufficient for JSON inputs. The framework parses JSON automatically, allowing attackers to bypass existence checks using truthy objects or arrays, which then crash string-specific method invocations.
**Prevention:** Always strictly validate the type of incoming request body properties (e.g., `typeof email === 'string'`) before invoking any string prototypes.
