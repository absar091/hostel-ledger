...
## 2026-08-23 - Prevent DoS from unchecked string methods on JSON payloads
**Vulnerability:** Express route handlers in `server.js` and `userRoutes.js` were reading properties like `req.body.email` and directly calling `.toLowerCase()` without verifying the type. If an attacker submits an array or object in the JSON payload (e.g. `{"email": []}`), this would cause a `TypeError` and crash the server, leading to a Denial of Service (DoS) vulnerability.
**Learning:** Node.js Express automatically parses JSON payloads into JavaScript objects, meaning client-submitted types cannot be trusted to be strings just because they conceptually represent string data.
**Prevention:** Always explicitly check `typeof value === 'string'` before calling string methods like `.toLowerCase()` or `.trim()` on user input extracted from `req.body`.
