...
## 2024-06-21 - Fix missing authentication on invitation endpoints
**Vulnerability:** The `/api/send-invitation` and `/api/send-external-invitation` endpoints in `backend-server/server.js` were missing the `authenticate` middleware, allowing unauthenticated users to trigger invitations, which could be abused for spam or enumeration.
**Learning:** In the Express backend, the `authenticate` middleware is not applied globally. Any endpoint that accesses `req.user` or triggers sensitive operations must explicitly include the `authenticate` middleware in its route definition to prevent authentication bypasses.
**Prevention:** Always verify that sensitive endpoints have the `authenticate` middleware explicitly added in their route definition, especially when adding new endpoints or refactoring.
