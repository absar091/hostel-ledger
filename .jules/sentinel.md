...
## 2024-05-24 - Firebase Realtime Database Group Creation Spoofing
**Vulnerability:** The database.rules.json validation for groups lacked a check ensuring newData.child("createdBy").val() === auth.uid on creation, allowing users to spoof group creation on behalf of any UID.
**Learning:** When creating records that contain a field representing the owner or creator, Firebase Realtime Database rules must explicitly validate that this field matches the authenticated users UID to prevent spoofing and unauthorized delegation.
**Prevention:** Always include (!data.exists() ? newData.child("createdBy").val() === auth.uid : true) in the .validate rule for collections where users act as creators.
