...
## 2026-03-24 - Path Traversal / IDOR in Budget Endpoints
**Vulnerability:** The `/api/budgets/group/:groupId` and `/api/budgets/personal/:userId` endpoints did not strictly validate URL parameters using `isValidFirebaseId` before placing them into Firebase realtime database query paths. Furthermore, the group budget endpoint failed to verify that the requesting user actually had access to the queried group via `userGroups/${req.user.uid}/${groupId}`, leading to Insecure Direct Object Reference (IDOR).
**Learning:** Always validate client-provided identifiers against strict regexes (e.g. `isValidFirebaseId`) to prevent NoSQL path traversal. Additionally, relying solely on `authenticate` middleware is insufficient; endpoints interacting with group data must independently query the database to verify the user’s group membership before executing operations.
**Prevention:** Explicitly validate URL route parameters (`req.params`) as strictly as `req.body` payloads. Verify group membership through a database query against `userGroups` to confirm access rights and prevent IDOR.
