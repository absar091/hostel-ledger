...
## 2025-05-24 - Validate targetId in User Reports
**Vulnerability:** The `/api/user/report` endpoint accepted the `targetId` parameter from the client and saved it directly to the database without validation.
**Learning:** Unvalidated referential IDs provided by untrusted clients can lead to stored malicious payloads or impact secondary contexts (like admin dashboards) if they are not restricted to the expected Firebase ID format.
**Prevention:** Always explicitly validate referential IDs using constraints like `isValidFirebaseId` upon receipt at the endpoint boundary before storing them in the database.
