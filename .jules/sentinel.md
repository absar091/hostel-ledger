...
## 2024-05-18 - [Fix rate limit bypass on API routes]
**Vulnerability:** The global rate limiter (`generalLimiter`) was mounted on `/api` after sensitive endpoints (such as `adminRoutes`, `userRoutes`, and `exportRoutes`) were already mounted. This routing order caused the rate limiter to be completely bypassed for those endpoints.
**Learning:** The order of middleware declaration in Express is extremely critical. Rate limiters and authentication checks must always precede the routes they intend to protect.
**Prevention:** Always verify that global middleware like `app.use('/api', generalLimiter);` is defined and placed before mounting any specific routers. Tests should specifically check the mounting order and ensure endpoints actually enforce the expected rate limits.
