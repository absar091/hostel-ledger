...
## 2026-03-18 - Path Traversal / NoSQL Injection in Budget Endpoints
**Vulnerability:** The `/api/budgets/group/:groupId` and `/api/budgets/group/:groupId` (POST) endpoints read `groupId` directly from `req.params` and interpolated it into Firebase database queries (`db.ref(\`budgets/${groupId}\`)` etc.) without validation. This allowed potential path traversal or NoSQL injection attacks.
**Learning:** Data from `req.params`, even when matched by route definitions, can still contain malicious path segments (e.g., encoded slashes or special characters) if not strictly validated before being used in database paths. However, an endpoint that implicitly relies on a strict check like `userId !== req.user.uid` may inherently protect against these payloads because the authorized uid does not contain malicious characters.
**Prevention:** Always explicitly validate parameters extracted from the request path (e.g., using `isValidFirebaseId`) before interpolating them into database queries or file paths, unless there is an implicit validation step (e.g., strict authorization check matching a known safe format).
