...
## 2024-05-25 - Open Relay Vulnerability in Support Email Endpoint
**Vulnerability:** The `/support` endpoint in `userRoutes.js` accepted an `email` parameter from the request body and used it directly as a fallback to send support confirmation emails without validating if it belonged to the authenticated user. This allowed any authenticated user to send support emails to arbitrary email addresses (acting as an open relay).
**Learning:** Endpoints that trigger email sending, even if authenticated, must strictly validate that the recipient matches the authenticated user's email to prevent abuse and spam. In this architecture, falling back to `req.body.email` when `req.user?.email` could be missing leads to an abuse vector.
**Prevention:** Always verify `req.body.email === req.user?.email` for self-targeted notifications, or strictly use `req.user.email` ignoring user-provided input. Also ensure email addresses are valid before attempting to send (e.g. check `email !== 'unknown'`).
