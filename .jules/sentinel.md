...
## 2024-05-18 - [Fix Open Relay and Type Injection in User Routes]
**Vulnerability:** The `/support` route was susceptible to email spoofing (Open Relay) by trusting `req.body.email` to determine the sender. Both `/support` and `/report` routes lacked strict type validation for incoming JSON body parameters, which exposed the server to DoS/NoSQL injection vulnerabilities if clients passed unexpected types like arrays or objects.
**Learning:** In Express apps using Firebase Authentication, always derive critical identifier fields (like `email` and `uid`) from the verified `req.user` token rather than client input. Additionally, even if inputs are seemingly straightforward (like `subject` or `message`), always enforce strict `typeof ... === 'string'` checks before further processing to prevent unexpected exceptions or injection.
**Prevention:** Standardize a middleware or utility pattern to enforce strict typing on all incoming JSON payloads, and strictly rely on decoded JWTs for authorization-sensitive fields rather than the request body.
