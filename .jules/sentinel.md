...
## 2024-05-25 - Prevent Information Leakage in Error Responses
**Vulnerability:** API endpoints were returning raw `error.message` to clients when internal server errors occurred.
**Learning:** Exposing backend error messages directly to the client can leak sensitive system architecture, database schema, or third-party service details to potential attackers.
**Prevention:** Always log the detailed error server-side and return generic, sanitized error messages to the client.
