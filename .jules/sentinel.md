...
## 2026-07-20 - Insecure Direct Object Reference / Privilege Escalation in Firebase Rules
**Vulnerability:** A missing validation rule on the `role` and `walletBalance` fields in the `users` collection allowed authenticated users to arbitrarily set their own role to 'admin' or 'superadmin' and modify their own wallet balance when creating or updating their user profile.
**Learning:** Firebase Realtime Database `.write` rules cascade. Setting `.write: false` on a child (e.g. `walletBalance`) does not prevent modification if the parent (e.g. `users/$uid`) has `.write: "$uid === auth.uid && auth != null"`. To block modification of specific child nodes when the parent is writable, `.validate` rules must be used since they are evaluated bottom-up.
**Prevention:** Always use `.validate` rules like `"!newData.exists() || (data.exists() && newData.val() === data.val())"` to strictly block client-side creation and modification of sensitive fields when parent nodes allow writes.
