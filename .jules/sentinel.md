...
## 2025-02-21 - [Missing Authentication on Push Endpoints]
**Vulnerability:** The `/api/push-subscribe`, `/api/push-notify`, and `/api/push-notify-multiple` endpoints were completely unauthenticated, allowing any external user to trigger push notifications to any user's device via the OneSignal REST API.
**Learning:** The `authenticate` middleware is not applied globally in the `server.js` Express application. Each endpoint that performs sensitive actions must explicitly include it.
**Prevention:** Always verify that newly added or existing endpoints that trigger external side-effects (like emails, push notifications, or database modifications) explicitly have the `authenticate` middleware in their route definition unless explicitly intended to be public.
