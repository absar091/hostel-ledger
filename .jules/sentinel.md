...
## 2026-09-07 - Node.js DoS via Unvalidated req.body Properties
**Vulnerability:** In multiple Express routes (`/api/send-welcome`, `/api/verification/request`, etc.), user input from `req.body` (like `email` or `username`) was destructured and immediately passed to string methods like `.toLowerCase()` or `.trim()`.
**Learning:** `body-parser` automatically parses JSON requests. An attacker can send a valid JSON object or array in place of a string (e.g., `{"email": {"$ne": null}}`). Because Objects evaluate as truthy (`if (!email)` passes), the subsequent `.toLowerCase()` call crashes the Node.js process with a `TypeError`. This is a classic Denial-of-Service (DoS) vector.
**Prevention:** Always explicitly validate the primitive type of the input (`if (typeof email !== 'string')`) before invoking any type-specific prototype methods on data originating from a client request payload.
