...
## 2025-01-20 - Privilege Escalation via User Profile Update
**Vulnerability:** The users Firebase realtime database rules allowed authenticated users to write to their own profile, without explicitly restricting sensitive fields like `role` or `accountStatus`. Using `.write: false` on child nodes failed because `.write` rules cascade. Furthermore, a naive `.validate` rule like `!data.exists() || newData.val() === data.val()` created a "write-once" vulnerability, allowing users to escalate privileges upon initial profile creation.
**Learning:** `.write` rules cascade in Firebase RTDB, meaning a parent's `.write: true` overrides child `.write: false` restrictions. To secure specific child fields while granting parent write access, use `.validate` rules. Additionally, `.validate` rules must be strictly formulated. If you want a field to be completely immutable by the client (even upon creation), you must use `.validate: "newData.val() === data.val()"`. This ensures that if the client sends a value, it must match the existing value (which is `null` if the field is new), effectively making it read-only for clients while remaining writable by the Admin SDK.
**Prevention:** Always restrict write access to `role`, `accountStatus`, `walletBalance`, and `settlements` using strict `.validate: "newData.val() === data.val()"` rules to prevent both modification and unauthorized initial creation by the client.
