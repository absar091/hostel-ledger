...
## 2026-03-24 - IDOR Vulnerability in AI Parse Expense Endpoints
**Vulnerability:** The `/api/ai/parse-expense` and `/api/ai/parse-expense-audio` endpoints fetched group data based on the provided `groupId` without verifying if the requesting user (`req.user.uid`) was actually a member of that group. This allowed any authenticated user to potentially extract data or perform actions on groups they didn't belong to.
**Learning:** Even AI-assisted endpoints or those that seem non-destructive can expose data if they handle group-scoped data without verifying the requesting user's authorization to access the referenced object.
**Prevention:** Always verify user membership against `userGroups/${req.user.uid}/${groupId}` prior to querying group-scoped data in AI endpoints.
