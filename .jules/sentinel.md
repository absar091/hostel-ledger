...
## 2024-05-25 - Privilege Escalation in User Profile
**Vulnerability:** The `/users/$uid` node in the Firebase Realtime Database rules allowed users to write to their own profile without validating the `role` and `accountStatus` fields. This allowed users to arbitrarily set their role to `admin` or unban their account.
**Learning:** Permissive `.write` rules on parent objects bypass restrictions on child fields unless explicitly constrained by `.validate` rules.
**Prevention:** Always add strict `.validate` rules to sensitive fields (like roles, permissions, or account statuses) to prevent client-side manipulation, even if the parent object requires user write access. Ensure rules explicitly define the allowed values during creation and strictly prevent modification during updates.
