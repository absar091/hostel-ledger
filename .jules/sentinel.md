...
## 2026-07-06 - Firebase Rules Cascading Write Escalation
**Vulnerability:** The users node allowed `$uid === auth.uid` for writes, but attempted to restrict `walletBalance` and `settlements` by setting `.write: false` on those child nodes. Due to Firebase's top-down cascading rules, the parent `.write` overrode the child `.write: false`, allowing users to overwrite their balances. Additionally, fields like `role` and `accountStatus` were entirely unrestricted, allowing any user to escalate their privilege to `admin` or modify their ban status.
**Learning:** In Firebase Realtime Database, `.write` and `.read` rules cascade and CANNOT be revoked at a lower level. To restrict specific fields when the parent allows writes, you MUST use `.validate` rules on those specific child fields, as `.validate` rules do not cascade and must all evaluate to true.
**Prevention:** Always use `.validate` to protect sensitive child fields inside user-writable documents (e.g., `role: { ".validate": "!newData.exists() || newData.val() === data.val()" }`).
