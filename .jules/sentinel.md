...
## 2024-05-25 - Privilege Escalation via Missing Field Validation in Firebase Rules
**Vulnerability:** Regular users had `.write` access to their entire user object (`users/$uid`), but there were no validation constraints protecting critical security fields like `role` or `accountStatus` within that object.
**Learning:** In Firebase Realtime Database, parent `.write` rules cascade and override child `.write` rules. Therefore, a user with `.write` access to a parent node can bypass child `.write` restrictions by performing a bulk update on the parent object.
**Prevention:** To securely restrict specific child fields (like `role` or `accountStatus`) when users have write access to the parent, you must use `.validate` rules on those child nodes. `.validate` rules evaluate independently of `.write` rules and must resolve to true for the specific data being written.
