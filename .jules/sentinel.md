...
## 2024-06-22 - Privilege Escalation via Client-Side Write Access to User Profile
**Vulnerability:** The `users/$uid` node in Firebase Realtime Database allowed authenticated users to write any field to their own profile, including sensitive fields like `role` and `accountStatus`. This allowed an attacker to escalate privileges to 'admin' or unban their account.
**Learning:** Parent `.write` rules cascade and cannot be overridden by child `.write: false` rules. To securely restrict specific child fields when the parent is writable by the client, you must use `.validate: "false"`.
**Prevention:** Always use `.validate` rules to block client-side modification of sensitive fields (like roles, statuses, balances) that are stored within client-writable parent nodes. Admin SDK operations will inherently bypass these rules.
