...
## 2024-05-25 - Privilege Escalation via Missing Firebase Realtime Database Validation Rules
**Vulnerability:** The Firebase Realtime Database rules allowed any authenticated user to write to their own `users/$uid` node. Because `.write` rules cascade top-down in Realtime Database, this parent `.write` permission overrode the missing or ineffective restrictions on child nodes like `role`, `walletBalance`, and `accountStatus`. This allowed users to elevate their privileges to `admin` or modify their `walletBalance` directly from the client.
**Learning:** In Firebase Realtime Database, parent `.write` rules override child `.write: false` restrictions. To secure specific child nodes when the parent is writable, `.validate` rules must be used, as they evaluate bottom-up and can block invalid schema or unauthorized data modifications.
**Prevention:** Always use `.validate` rules on sensitive child fields (e.g., `role`, `walletBalance`) to restrict modifications, even when the parent node allows writes.
