...
## 2024-07-04 - [Fix reverse tabnabbing]
**Vulnerability:** Found `<a>` tags with `target="_blank"` missing the `rel="noopener noreferrer"` attribute.
**Learning:** Without `rel="noopener noreferrer"`, the newly opened page can access the original page's window object using `window.opener`, which is a security risk known as reverse tabnabbing. It can allow the new page to navigate the original page to a malicious URL.
