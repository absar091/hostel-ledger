...
## 2024-05-25 - Reverse Tabnabbing Vulnerability
**Vulnerability:** Found `<a>` tags utilizing `target="_blank"` for internal routing without the `rel="noopener noreferrer"` attribute in `src/components/CreateGroupSheet.tsx` and `src/pages/CreateGroup.tsx`.
**Learning:** Even internal links to terms of service or privacy policies can be exploited if the target page is compromised or if it redirects to an external, untrusted site. Without `rel="noopener noreferrer"`, the newly opened page retains a reference to the `window.opener` object, allowing it to maliciously redirect the original page (reverse tabnabbing).
**Prevention:** When using standard `<a>` tags with `target="_blank"`, consistently append the `rel="noopener noreferrer"` attribute, regardless of whether the link points internally or externally.
