...
## 2026-09-04 - Open Relay Vulnerability in Support Ticket Endpoint
**Vulnerability:** The `/support` endpoint in `backend-server/routes/userRoutes.js` accepted an `email` parameter from the request body and used it directly in a support ticket email notification without validating if it belonged to the authenticated user. This allowed any authenticated user to send support emails that appeared to come from arbitrary email addresses.
**Learning:** Endpoints that trigger email sending must strictly validate that the recipient matches the authenticated user's email to prevent spoofing and abuse.
**Prevention:** Always use `req.user.email` (from the authenticated session) instead of trusting `req.body.email` when sending system emails or submitting tickets on behalf of a user.
