...
## 2024-05-25 - Predictable Notification IDs using Date.now()
**Vulnerability:** Notification IDs for broadcast and ticket replies were generated using only `Date.now()`, making them predictable and susceptible to enumeration or ID collisions.
**Learning:** Relying purely on timestamps for ID generation is insecure and creates predictability that attackers can use to guess identifiers.
**Prevention:** Always use cryptographically secure methods like `crypto.randomBytes(4).toString('hex')` in combination with or instead of timestamps to generate IDs or tokens.
