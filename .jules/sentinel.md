...
## 2024-05-18 - Prevent Privilege Escalation in Firebase Rules
**Vulnerability:** The `users/$uid` node in `database.rules.json` had a parent `.write` rule allowing users to modify their own data, while attempting to restrict specific fields (`walletBalance`, `settlements`) using child `.write: false` rules. However, Firebase Realtime Database `.write` rules cascade top-down, meaning the parent's `true` completely overrides the child's `false`, exposing these fields to arbitrary modification. Additionally, sensitive fields like `role` and `accountStatus` were missing restrictions, allowing privilege escalation during user creation.
**Learning:** You cannot restrict child paths using `.write` if a parent path grants write access.
**Prevention:** Use `.validate` rules on child nodes (e.g., `".validate": "newData.val() === data.val()"`) to strictly restrict modifications, as `.validate` rules do not cascade and must all independently evaluate to true.
