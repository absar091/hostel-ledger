...
## 2024-05-24 - Firebase Rules Cascading Write Bypass
**Vulnerability:** The users node in Firebase RTDB had `.write` access for the authenticated user, but attempted to restrict child nodes (`walletBalance`, `settlements`) using `.write: false`. However, `.write` and `.read` rules in Firebase RTDB cascade. If a parent grants access, child restrictions are ignored. This allowed users to arbitrarily modify their wallet balance and settlements. In addition, there was no protection preventing users from creating themselves as an admin or updating their role.
**Learning:** Never rely on `.write: false` or `.read: false` on child nodes if the parent node grants access.
**Prevention:** Always use `.validate` rules to enforce schema and immutability on child fields when the parent node allows writes. `.validate` rules do not cascade and are enforced independently of `.write` rules. For write-once fields, use rules like `(!data.exists() && newData.val() === 'expected') || (data.exists() && newData.val() === data.val())`.
