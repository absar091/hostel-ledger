...
## 2026-07-18 - Fix privilege escalation and write bypass in Firebase DB rules
**Vulnerability:** Users could escalate their privileges to admin and modify restricted fields like walletBalance.
**Learning:** In Firebase Realtime Database, `.write` rules cascade top-down and child rules cannot override parent grants. The `.write: false` on child nodes was completely ignored.
**Prevention:** Always use `.validate` rules to enforce immutability or restrict specific fields when the parent node grants write access.
