...
## 2024-05-27 - [Type validation for email inputs in Express routes]
**Vulnerability:** Found multiple sensitive endpoints (password reset, 2FA, verifications) where `req.body.email` was expected to be a primitive string but could be submitted as a JSON object or array. This payload would then be passed to `email.toLowerCase()`, throwing a `TypeError` and causing a Denial of Service (DoS) crash for the handler.
**Learning:** Destructuring variables from Express `req.body` does not inherently guarantee they are primitives. Without explicit `typeof === 'string'` checks, attackers can submit unexpected types (like objects) to cause runtime exceptions.
**Prevention:** Always validate that the type of extracted variables from `req.body` matches the expected primitive type before calling prototype methods like `.toLowerCase()` or `.replace()`.
