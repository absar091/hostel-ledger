...
## 2024-05-18 - Firebase Database Rules Parent Write Override
**Vulnerability:** Ineffective child write restrictions allowing privilege escalation.
**Learning:** In Firebase Realtime Database, `.write` rules cascade top-down. If a parent node (like `users/$uid`) grants `.write: true`, any child nodes (like `walletBalance`) with `.write: false` are completely overridden and ignored.
**Prevention:** To restrict specific fields when parent write access is granted, you MUST use `.validate` rules on the parent or child, because `.validate` rules do not cascade and must explicitly pass for the write to succeed.
