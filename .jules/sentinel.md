...
## 2024-05-24 - Support Ticket Open Relay
**Vulnerability:** Support ticket endpoint used user-provided email address in the request body to send confirmation emails without verification.
**Learning:** Endpoints that send automated emails must never trust user-provided email fields if an authenticated user context is available.
**Prevention:** Always use the authenticated user's email from the validated context (e.g., `req.user.email`) to prevent Open Relay and spam abuse vulnerabilities.
