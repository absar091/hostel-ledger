...
## 2025-02-18 - Open Relay / Spam Abuse in Support Ticket Endpoint
**Vulnerability:** The `/support` endpoint allowed users to specify arbitrary email addresses in the `req.body` to receive support ticket confirmation emails.
**Learning:** This exposes the application to Open Relay and spam abuse vulnerabilities, where attackers could use the endpoint to send automated emails to victim addresses.
**Prevention:** Always rely strictly on the authenticated user's context (e.g., `req.user.email`) for system-generated outbound communication, rather than trusting client-provided email fields.
