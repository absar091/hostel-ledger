...
## 2025-02-27 - Fix Authentication Bypass and Middleware Ordering
**Vulnerability:** Found two API endpoints (`/api/send-invitation` and `/api/send-external-invitation`) completely missing the `authenticate` middleware. Additionally, the custom `detectFraud` middleware was globally bypassed because it was ordered before `authenticate` in the route definitions, causing it to prematurely exit since `req.user` was undefined.
**Learning:** Middleware execution order is critical. Security mechanisms relying on authentication context (like checking `req.user.uid` in `detectFraud`) will silently fail or become no-ops if placed before authentication middleware.
**Prevention:** Always verify middleware chains in Express routers to ensure that `authenticate` is explicitly included on sensitive routes and that it precedes any custom middleware expecting an authenticated context.
