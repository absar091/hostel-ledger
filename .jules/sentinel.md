...
## 2026-04-29 - NoSQL Injection in get-transaction-preview
**Vulnerability:** The `/api/get-transaction-preview` endpoint took `transactionId` directly from `req.body` and interpolated it into a Firebase database path without validation.
**Learning:** Even simple preview or read-only endpoints are susceptible to NoSQL injection and path traversal if user-provided identifiers are not validated before being used in database queries.
**Prevention:** Always validate client-provided IDs using `isValidFirebaseId` before using them in database paths.
