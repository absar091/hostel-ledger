...
## 2024-05-24 - Fix privilege escalation and balance modification in Firebase rules
**Vulnerability:** Firebase rules for `users/$uid` had cascading `.write` access. While `walletBalance` and `settlements` had `.write: false`, this does not restrict writes when parent `.write` is granted. Also, there was no restriction on the `role` field.
**Learning:** In Firebase Realtime Database, `.write` and `.read` rules cascade top-down. To strictly restrict write access to specific child fields when the parent node grants write access, you must use `.validate` rules because they do not cascade.
**Prevention:** Apply `.validate: "!newData.exists() || (data.exists() && newData.val() === data.val())"` to child fields that should never be written by the client, or restrict specific values during creation and modification.
