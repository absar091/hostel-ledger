...
## 2025-05-18 - Missing XSS Sanitization in dangerouslySetInnerHTML
**Vulnerability:** Found `dangerouslySetInnerHTML` in `src/components/ui/chart.tsx` injecting theme variables directly into a `<style>` block without sanitization. While themes are currently hardcoded, any future dynamic CSS injection could introduce DOM-based XSS.
**Learning:** `dangerouslySetInnerHTML` is easily overlooked when injecting CSS properties.
**Prevention:** Avoid injecting unsanitized structural strings into `<style>`.
