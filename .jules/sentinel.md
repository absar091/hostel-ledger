...
## 2025-03-09 - Ineffective Child Write Rules in Firebase
**Vulnerability:** The users/$uid node had child write restrictions (.write: false) on walletBalance and settlements, but users had full write access to their parent node (users/$uid).
**Learning:** In Firebase Realtime Database, parent .write permissions cascade to all child nodes and cannot be revoked by child .write rules. Therefore, a user can bypass child write restrictions by updating the entire parent object.
**Prevention:** Always use .validate rules to enforce specific data restrictions (like immutability) when the user has write access to the parent node, as .validate rules are evaluated at the node they are defined and cannot be bypassed by parent write rules.
