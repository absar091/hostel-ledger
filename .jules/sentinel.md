...
## 2024-05-24 - Firebase Privilege Escalation via Parent Write Cascading
**Vulnerability:** Client-side privilege escalation and tampering of sensitive fields (`role`, `accountStatus`, `walletBalance`, `settlements`) within the `users/$uid` node.
**Learning:** In Firebase Realtime Database, a parent `.write` rule (e.g., `"$uid === auth.uid"`) cascades and completely overrides any child `.write` restrictions (e.g., `".write": false`). Users with parent write access can modify any child field.
**Prevention:** To restrict specific child nodes when the parent is writable, use `.validate` rules (e.g., `".validate": "false"`) instead of `.write` rules. `.validate` rules cannot be bypassed by parent write permissions. Admin server SDKs will inherently bypass both rules.
