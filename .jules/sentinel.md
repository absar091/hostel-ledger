...
## 2026-03-18 - Missing Firebase ID Validation on Group Budget Endpoints
**Vulnerability:** Unvalidated user input from `req.params.groupId` was directly interpolated into Firebase Realtime Database paths (e.g., `db.ref('budgets/${groupId}')`) in the group budget management endpoints.
**Learning:** Even authenticated endpoints can be vulnerable to path traversal or NoSQL injection if path parameters used to query shared resources are not strictly validated. However, validating parameters like `userId` that are immediately strictly compared against a trusted server value (`req.user.uid`) is unnecessary security theater.
**Prevention:** Always explicitly validate path parameters for shared resources (e.g., using `isValidFirebaseId`) before executing database queries. Avoid redundant validation when strict equality checks against trusted data already provide complete protection.
