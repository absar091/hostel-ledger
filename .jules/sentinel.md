...
## 2024-08-02 - Fix IDOR in AI Expense Parsing Endpoints
**Vulnerability:** The `/api/ai/parse-expense` and `/api/ai/parse-expense-audio` endpoints allowed any authenticated user to retrieve group context details and query group expenses without verifying if they were actually a member of that group, resulting in an Insecure Direct Object Reference (IDOR) vulnerability.
**Learning:** Endpoints that require group IDs and use those IDs to pull context from the database must verify membership via `userGroups/${req.user.uid}/${groupId}` before performing actions or returning data related to that group. Relying on simple authentication without checking authorization leads to IDOR.
**Prevention:** Always verify group membership using `userGroups` table checks at the beginning of group-specific endpoints to ensure the requesting user has the right to access that resource.
