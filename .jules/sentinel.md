...
**Vulnerability:** External links with `target="_blank"` missing `rel="noopener noreferrer"` attribute.
**Learning:** Found instances of this vulnerability in React components using raw `<a>` tags for external links instead of a hardened custom component. While these links pointed to internal sub-domains, it is a bad practice.
**Prevention:** Always include `rel="noopener noreferrer"` when using `target="_blank"`. Consider creating a unified `ExternalLink` component to enforce this globally.
