...
## 2024-05-25 - Client-side privilege escalation via unvalidated node rewrites
**Vulnerability:** Firebase Realtime Database rules permitted users to write their entire user profile node (e.g., `users/$uid`) without validating that sensitive fields like `role` or `accountStatus` remained unchanged. This allowed malicious actors to escalate their privileges to 'admin' or reactivate banned accounts by including these fields in a profile update.
**Learning:** In Firebase Realtime Database, a broad parent `.write` rule enables clients to overwrite the entire node, effectively wiping out or modifying any child properties. Parent write rules cannot be revoked at the child level (i.e., `".write": false` on `role` is ignored if the parent allows the write).
**Prevention:** To protect specific sensitive child fields when the parent node is writable, you must use `.validate` rules on those child fields. A rule like `".validate": "data.val() === newData.val()"` prevents the client from altering the existing value, while the backend Admin SDK inherently bypasses all rules to perform necessary updates.
