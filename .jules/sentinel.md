...
## 2025-02-26 - IDOR in Miscellaneous Group Endpoints
**Vulnerability:** Several group-scoped endpoints (e.g., `/api/ai/parse-expense`, `/api/add-expense`, `/api/get-messages`) queried group data or performed actions using `groupId` without explicitly verifying if the requesting user (`req.user.uid`) was actually a member of the group, leading to Insecure Direct Object Reference (IDOR).
**Learning:** Even if `isValidFirebaseId(groupId)` is checked, authentication (`req.user`) does not automatically imply authorization for a specific resource unless `userGroups/${req.user.uid}/${groupId}` is strictly verified.
**Prevention:** Always ensure an explicit authorization check (`admin.database().ref('userGroups/${req.user.uid}/${groupId}').once('value')`) is performed before fulfilling requests on group-scoped data in backend endpoints.
