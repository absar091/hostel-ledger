...
## 2026-09-05 - Fix Missing Rate Limits on Sensitive API Routes
**Vulnerability:** The Express.js router for `/api/admin`, `/api/user`, and `/api/export` was mounted before the `generalLimiter` middleware. This caused these critical routes to completely bypass rate limiting, making them susceptible to DoS and brute-force attacks.
**Learning:** In Express.js, the order of `app.use()` directly determines the middleware stack. Middlewares registered after a route group do not apply to that group.
**Prevention:** Always verify that global security middlewares (like rate limiters and authentication) are initialized and mounted via `app.use()` *before* any sub-routers that require their protection are mounted.
