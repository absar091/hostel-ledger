...
## 2024-05-18 - Express Middleware Mounting Order
**Vulnerability:** Global rate limiting and authentication middleware were mounted *after* some API routers, completely bypassing security controls for those routes.
**Learning:** Express executes middleware sequentially. Routes mounted before a global middleware are entirely unaffected by it. This meant `/api/admin`, `/api/user`, and `/api/export` had no rate limiting, and `/api/admin` bypassed global API auth entirely (relying only on its internal checks).
**Prevention:** Always mount global security middleware (`rateLimit`, `authenticate`) immediately after parsers and basic setup, strictly *before* mounting any routers they are meant to protect.
