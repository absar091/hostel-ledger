...
## 2024-05-27 - Open Relay via Support Endpoint & DoS via Unvalidated Type
**Vulnerability:** The `/support` endpoint in `backend-server/routes/userRoutes.js` allowed users to provide an arbitrary `email` address in the request body, which the system would use to send a support confirmation email. This acted as an open relay since it was not verified against the authenticated user's email. Furthermore, a naive fix attempting to call `.toLowerCase()` on the unvalidated `email` input would have introduced a DoS vulnerability (unhandled exception if the input was an array or object).
**Learning:** Endpoints that trigger emails must verify the target address against the authenticated user's context (e.g., `req.user.email`) to prevent abuse. Additionally, any user input from `req.body` must be type-checked (e.g., `typeof value === 'string'`) before invoking string methods to prevent unhandled exceptions.
**Prevention:** Always verify `req.body.email` against `req.user.email` for self-targeted communications. Always validate input data types before operating on them.
