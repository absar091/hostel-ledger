...
## 2024-05-24 - [Strict Type Validation for Request Body Strings]
**Vulnerability:** Node.js Express server routes accepted user input (like `email`, `code`, `username`) from `req.body` and immediately executed string-specific methods like `.toLowerCase()` and `Buffer.from()` without verifying if the parsed JSON value was actually a string.
**Learning:** If an attacker crafts a JSON payload providing an object or array instead of a string (e.g., `{"email": ["admin@example.com"]}`), calling `.toLowerCase()` on it causes an unhandled `TypeError` exception. In Express, unhandled asynchronous errors can crash the entire Node.js server process (Denial of Service). Furthermore, object payloads can sometimes bypass rudimentary falsy checks and cause NoSQL injection depending on the database adapter.
**Prevention:** Always implement explicit type checking (`typeof value === 'string'`) in route handlers for untrusted user input originating from parsed JSON bodies before invoking string methods or using the variables in database queries.
