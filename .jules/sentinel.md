...
## 2024-03-05 - Rate Limiting Bypass on Modular Routes
**Vulnerability:** Express modular routes (`/api/admin`, `/api/user`, `/api/export`) were mounted before the `generalLimiter` rate limit middleware was applied to `/api`, bypassing rate limiting.
**Learning:** Middleware ordering in Express is critical. Global route protections must be initialized and mounted before any specific routes that fall under their path prefix.
**Prevention:** Always mount security middleware (rate limiters, auth) before defining or mounting the routes they protect.
