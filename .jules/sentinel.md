...
## 2026-08-07 - Open Relay in Support Ticket creation
**Vulnerability:** The `/support` endpoint extracted the `email` directly from `req.body` to use as the sender address when logging a ticket and sending the auto-confirmation email. An attacker could provide any arbitrary email, forcing the system to send support confirmation emails to unconsenting victims (Open Relay / Mail Bombing vector) or spoof tickets on behalf of administrators.
**Learning:** Endpoints that trigger emails must never trust client-provided email addresses if an authenticated user session is available. Doing so enables attackers to exploit the server's email reputation.
**Prevention:** Strictly rely on the authenticated context (e.g., `req.user.email`) for the sender's identity when generating support tickets or similar emails, ignoring any client-provided email fields.
