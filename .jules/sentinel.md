...
## 2026-03-24 - Open Relay and Spam Abuse via Support Endpoint
**Vulnerability:** The `/support` endpoint in `backend-server/routes/userRoutes.js` accepted an `email` string parameter directly from the request body (`req.body.email`). If provided, it overrode the authenticated user's email (`req.user.email`) when constructing the `ticketData` payload. Consequently, `emailService.sendEmail` would send an automated confirmation message to any arbitrary email address submitted by an attacker, turning the endpoint into an open relay for spam and phishing.
**Learning:** Endpoints that trigger transactional or automated emails must strictly isolate the recipient address from untrusted client input, especially in authenticated contexts where the user's identity is already known via JWT or session.
**Prevention:** Never trust `req.body.email` for notifications meant for the current user. Always extract the user's email exclusively from the validated authentication context (e.g., `req.user.email`).
