...
## 2025-02-27 - Fix Open Relay Vulnerability in Support Endpoint
**Vulnerability:** The `/api/support` endpoint used a user-provided email payload to send confirmation emails, falling back to the authenticated user's email only if the payload was missing. This allowed authenticated users to spoof emails or use the system as an open relay to spam arbitrary email addresses.
**Learning:** Prioritizing untrusted client input over verifiable server-side claims (like JWT tokens) for email delivery creates abuse vectors.
**Prevention:** Always strictly use the authenticated user's email (`req.user?.email`) for automated communications when available, and only fallback to provided input if the user is unauthenticated or the token lacks the claim.
