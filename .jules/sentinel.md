...
## 2025-05-15 - [Path Traversal in Unauthenticated Endpoint]
**Vulnerability:** Found NoSQL path traversal via unsanitized uid parameter in unauthenticated 2FA reset endpoint /api/2fa/complete-reset.
**Learning:** External IDs passed directly into admin.database().ref() allow directory traversal if not validated, bypassing the root node context.
**Prevention:** Always sanitize/validate path parameters using isValidFirebaseId() prior to interpolating them into Firebase Admin SDK database references.
