...
## 2026-06-17 - Insecure Support Ticket ID Entropy
**Vulnerability:** Support ticket IDs were generated using `crypto.randomBytes(3).toString('hex').toUpperCase()`, resulting in only a 24-bit identifier (6 characters, ~16 million possibilities). This low entropy makes ticket IDs susceptible to brute-forcing or enumeration if they are ever used as a security boundary or lookup mechanism without strong secondary authorization.
**Learning:** While `crypto.randomBytes` is cryptographically secure, using too few bytes provides insufficient entropy for resource identifiers, creating predictability vulnerabilities similar to `Math.random()`.
**Prevention:** Always ensure cryptographically secure random identifiers have sufficient byte length (e.g., at least 8-16 bytes for standard IDs, or standard UUIDs) to prevent enumeration.

## 2026-06-17 - Unvalidated targetId in /report Endpoint leading to NoSQL Injection
**Vulnerability:** The `/report` endpoint accepted a `targetId` directly from the request body without verifying its format. While it was not immediately interpolated into a vulnerable path in this specific endpoint block, accepting an unvalidated ID string into the database (`reportRef.set({ targetId })`) poses a significant risk if that `targetId` is later consumed or queried by administrative tools or other endpoints.
**Learning:** Any client-provided identifier that references a database object must be strictly validated before being accepted into the system, even if the immediate endpoint only stores it, to prevent second-order injection or path traversal downstream.
**Prevention:** Always validate client-provided reference IDs (e.g., using `isValidFirebaseId`) immediately at the API boundary before processing or storing them.
