...
## 2026-07-04 - Privilege Escalation via User Profile Update
**Vulnerability:** A user's profile node in Firebase Realtime Database (`users/$uid`) allowed them to write arbitrary data under their own node due to the `.write: "$uid === auth.uid"` rule. This allowed any user to escalate their privileges by self-assigning `role: 'admin'` or `role: 'superadmin'`, or changing their `accountStatus` bypassing administrative controls.
**Learning:** In Firebase Realtime Database, granting write access to a parent node grants write access to all child nodes by default. Without strict `.validate` rules on sensitive child fields like `role`, users can add or modify those fields arbitrarily.
**Prevention:** Always explicitly define `.validate` rules for sensitive fields within user-editable nodes, ensuring they can only be created with default values (e.g., `'user'`) and cannot be modified by the user once they exist.
