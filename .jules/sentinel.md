...
## 2024-05-24 - Privilege Escalation via Missing Field Validation in Firebase Rules
**Vulnerability:** The users collection allowed users to write arbitrary fields under their uid, lacking validation on the 'role' field, which defaults to admin in absence of constraints.
**Learning:** In Firebase Realtime Database rules, missing validation rules on specific child properties allows unrestricted data creation by the object owner, leading to privilege escalation.
**Prevention:** Explicitly validate all security-critical properties (e.g., 'role') natively in Firebase rules to enforce secure default values, ensuring only authorized roles (like admin/superadmin) can escalate privileges.
