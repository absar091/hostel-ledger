...
## 2024-06-19 - Ineffective Firebase Child Node Write Restrictions
**Vulnerability:** In Firebase Realtime Database, using `.write: false` on child nodes (like `walletBalance`) to restrict access while granting `.write: true` on the parent node (`/users/$uid`) allows a malicious client to overwrite the parent node and completely bypass the child node restrictions. This also allowed client-side privilege escalation by injecting `role` and `accountStatus`.
**Learning:** Firebase parent `.write` rules cascade and cannot be revoked by child `.write` rules. A parent permission guarantees write access to the entire subtree.
**Prevention:** To restrict specific fields while allowing parent writes, use `.validate` rules on the child nodes instead of `.write` rules, because `.validate` rules are evaluated independently and cannot be bypassed by parent permissions. Backend Admin operations naturally bypass these rules, so no complex logic is needed for administrative updates.
