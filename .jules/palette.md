...
## 2024-05-20 - Adding Tooltips to Radix Modals
**Learning:** When adding standard Radix UI `<Tooltip>` components inside portals or modals (like the `TransactionDetailModal` overlay) that aren't wrapped in a global app-level provider, the tooltips may fail to render or position correctly.
**Action:** Always explicitly wrap the component tree inside the modal (e.g., inside the main return fragment) with a `<TooltipProvider>` to ensure local tooltips function.
