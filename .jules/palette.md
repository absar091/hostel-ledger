...
## 2024-03-24 - Screen Reader Support for Icon-only Buttons
**Learning:** Found an accessibility issue pattern specific to this app where icon-only buttons lacked aria-labels, making them completely inaccessible to screen readers since they have no readable text content.
**Action:** Always ensure that icon-only action buttons (like X to dismiss, image attachments, etc.) have explicit \`aria-label\` attributes that describe their action to assist screen reader users.
