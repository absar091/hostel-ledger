...
const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  const { user, firebaseUser, isLoading, is2FAVerified } = useFirebaseAuth();
  ...
  if (!user) {
    ...
    // If user is authenticated but profile failed to load
    if (firebaseUser) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-white">
          ...
          <p className="text-gray-600 mb-6 text-center max-w-sm">
            We could not load your profile data. This might be due to a poor connection.
          </p>
          <Button onClick={() => window.location.reload()} className="bg-[#4a6850] hover:bg-[#3d5643]">
            Retry Connection
          </Button>
        </div>
      );
