...
const TimelineItemBase = ({
  ...
  if (type === "wallet_add") {
    return (
      <button
        onClick={onClick}
        aria-label={`Added to wallet: ${title}, amount ${formatAmount(amount)} on ${date}`}
  ...
  if (type === "wallet_deduct") {
    return (
      <button
        onClick={onClick}
        aria-label={`Deducted from wallet: ${title}, amount ${formatAmount(amount)} on ${date}`}
  ...
  if (type === "payment") {
    ...
    return (
      <button
        onClick={onClick}
        aria-label={`${isPayer ? 'Payment Sent' : 'Payment Received'} from ${from} to ${to}, amount ${formatAmount(amount)} on ${date}`}
  ...
  return (
    <button
      onClick={onClick}
      aria-label={`Expense: ${title}, paid by ${payers && payers.length > 1 ? `${payers.length} people` : paidBy}, amount ${formatAmount(amount)} on ${date}`}
