    ...
    <aside className={cn(
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        {navItems.map((item: any) => {
          ...
          return (
            <TooltipProvider key={item.id}><Tooltip delayDuration={0}><TooltipTrigger asChild><button
            ...
            </button></TooltipTrigger>{!isOpen && <TooltipContent side="right" sideOffset={20}>{item.label}</TooltipContent>}</Tooltip></TooltipProvider>
          );
        })}
      </nav>
      ...
      <div className="p-4 border-t border-gray-200">
        {isOpen ? (
          ...
          <>
            <TooltipProvider><Tooltip delayDuration={0}><TooltipTrigger asChild><button
            ...
            </button></TooltipTrigger><TooltipContent side="right" sideOffset={20}>{user?.name || "Profile"}</TooltipContent></Tooltip></TooltipProvider>
            <TooltipProvider><Tooltip delayDuration={0}><TooltipTrigger asChild><button
              onClick={handleLogout}
            ...
            >
              <LogOut className="w-5 h-5" />
            </button></TooltipTrigger><TooltipContent side="right" sideOffset={20}>{t(\'sidebar.logout\')}</TooltipContent></Tooltip></TooltipProvider>
          </>
        )}
      </div>
    </aside>
