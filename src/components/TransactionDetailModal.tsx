...
const TransactionDetailModal = ({
  ...
  const participantsList = Array.isArray(transaction.participants)
    ...
    : transaction.participants
      ? Object.entries(transaction.participants).map(
  ...
  const fromMember = transactionGroup?.members.find(
    (m: any) => m.id === transaction.from,
  );
  const toMember = transactionGroup?.members.find(
    (m: any) => m.id === transaction.to,
  );
  const resolvedFromName =
    transaction.from === user?.uid || fromMember?.userId === user?.uid
      ? "You"
      : fromMember?.name || transaction.fromName || "Unknown";
  const resolvedToName =
    transaction.to === user?.uid || toMember?.userId === user?.uid
      ? "You"
      : toMember?.name || transaction.toName || "Unknown";
  ...
  const receiptDate =
    ...
    (transaction.timestamp
      ? ` • ${new Date(transaction.timestamp).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })}`
      : "");

  return (
    <>
      <div
        className={`fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 ${showChat ? "hidden" : ""}`}
      >
        <div className="bg-white w-full max-w-md max-h-[90vh] overflow-hidden rounded-3xl shadow-[0_25px_70px_rgba(74,104,80,0.3)] border border-[#4a6850]/10 mx-auto">
          ...
          <div className="flex items-center justify-between p-4 lg:p-6 border-b border-[#4a6850]/10 bg-gradient-to-r from-[#4a6850]/5 to-[#3d5643]/5 flex-shrink-0">
            <div className="flex items-center gap-3 lg:gap-4 flex-1 min-w-0">
              <div
                className={`w-10 lg:w-12 h-10 lg:h-12 rounded-2xl lg:rounded-3xl flex items-center justify-center shadow-lg flex-shrink-0 ${
              ...
              <div className="min-w-0 flex-1">
                <h2 className="font-bold text-gray-900 text-base lg:text-lg tracking-tight truncate">
                  Transaction Details
                </h2>
                <p className="text-xs lg:text-sm text-[#4a6850]/80 capitalize font-medium truncate">
                  {transaction.type}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 lg:gap-3 ml-3 lg:ml-4">
              ...
              <Tooltip delayDuration={0}>
                ...
                <TooltipContent side="bottom">Share as Image</TooltipContent>
              </Tooltip>
              <button
                onClick={onClose}
              ...
              >
                <X
                  className="w-4 lg:w-5 h-4 lg:h-5 text-white font-bold"
                  strokeWidth={3}
                />
              </button>
            </div>
          ...
          >
            <div className="p-4 lg:p-6">
              ...
              <div className="text-center mb-6 lg:mb-8">
                <h3 className="text-lg lg:text-2xl font-bold text-gray-900 mb-2 lg:mb-3 tracking-tight truncate px-2">
                  {transaction.title}
                </h3>
                ...
                {transaction.type === "expense" && !isCurrentUserPayer && (
                  <div className="mb-2">
                    {(() => {
                      ...
                      const userPart = resolvedParticipants.find(
                        (p: any) =>
                          p.id === user?.uid ||
                          (p.userId && p.userId === user?.uid),
                      );
                        ...
                        return (
                          <div className="inline-flex items-center px-3 py-1 rounded-full bg-rose-50 text-rose-600 border border-rose-100 shadow-sm">
                            <span className="text-xs font-black uppercase tracking-wider">
                              Your Share: {formatAmount(userPart.amount)}
                            </span>
                          </div>
                        );
                      } else {
              ...
              <div className="space-y-3 lg:space-y-4">
                ...
                {transactionGroup && (
                  <div className="flex items-center gap-3 lg:gap-4 p-4 lg:p-5 bg-gradient-to-br from-[#4a6850]/5 to-[#3d5643]/5 rounded-2xl lg:rounded-3xl border border-[#4a6850]/20 shadow-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[10px] lg:text-xs text-[#4a6850]/70 font-semibold uppercase tracking-wide">
                        Group
                      </div>
                      ...
                      <div className="text-xs lg:text-sm text-[#4a6850]/80 font-medium">
                        {Array.isArray(transactionGroup.members)
                          ...
                          : Object.keys(transactionGroup.members || {})
                              .length}{" "}
                        members
                      </div>
                    </div>
                ...
                {isMultiPayer && (
                  <div className="p-4 lg:p-5 bg-gradient-to-br from-[#4a6850]/5 to-[#3d5643]/5 rounded-2xl lg:rounded-3xl border border-[#4a6850]/20 shadow-lg">
                    <div className="text-[10px] lg:text-xs text-[#4a6850]/70 mb-3 lg:mb-4 font-semibold uppercase tracking-wide">
                    ...
                    <div className="space-y-2 lg:space-y-3 max-h-32 overflow-y-auto scrollbar-hide">
                      {resolvedPayers.map((payer: any, index: number) => {
                        const isOwner =
                          transactionGroup?.createdBy === payer.id;
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                              ...
                              {isOwner && (
                                <span className="bg-yellow-100 text-yellow-700 text-[8px] px-1 rounded font-black border border-yellow-200 uppercase tracking-wide">
                                  Owner
                                </span>
                              )}
                            </div>
                            <span className="text-xs lg:text-sm text-[#4a6850] flex-shrink-0 font-bold tabular-nums">
                  ...
                  <div className="flex items-center gap-3 lg:gap-4 p-4 lg:p-5 bg-gradient-to-br from-[#4a6850]/5 to-[#3d5643]/5 rounded-2xl lg:rounded-3xl border border-[#4a6850]/20 shadow-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="text-[10px] text-gray-400 font-bold uppercase tracking-widest pl-1">
                          TO
                        </div>
                      ...
                      {transaction.method && (
                        <div className="text-xs lg:text-sm text-[#4a6850]/80 capitalize font-medium mt-2">
                ...
                {transaction.participants &&
                  transaction.participants.length > 0 && (
                    <div className="p-4 lg:p-5 bg-gradient-to-br from-[#4a6850]/5 to-[#3d5643]/5 rounded-2xl lg:rounded-3xl border border-[#4a6850]/20 shadow-lg">
                      ...
                      <div className="space-y-2 lg:space-y-3 max-h-32 overflow-y-auto scrollbar-hide">
                        {resolvedParticipants?.map(
                          (participant: any, index: number) => {
                            const isOwner =
                              transactionGroup?.createdBy === participant.id;
                            const isMe =
                      ...
                      <div className="mt-3 flex gap-4 bg-white/50 p-3 rounded-2xl border border-[#4a6850]/10 shadow-sm">
                        ...
                        <div className="flex flex-col justify-center gap-2">
                          <span className="text-xs text-[#4a6850]/80 font-mono font-bold bg-[#4a6850]/5 px-2 py-1 rounded-lg border border-[#4a6850]/10">
                            {transaction.location.lat.toFixed(4)},{" "}
                            {transaction.location.lng.toFixed(4)}
                          </span>
                          <button
                            onClick={() => {
                              const url = `https://www.google.com/maps/search/?api=1&query=${transaction.location?.lat},${transaction.location?.lng}`;
                ...
                {transaction.note && (
                  <div className="p-4 lg:p-5 bg-gradient-to-br from-[#4a6850]/5 to-[#3d5643]/5 rounded-2xl lg:rounded-3xl border border-[#4a6850]/20 shadow-lg">
                    <div className="text-[10px] lg:text-xs text-[#4a6850]/70 mb-2 lg:mb-3 font-semibold uppercase tracking-wide">
                      Note
                    </div>
                    <div className="font-medium text-gray-900 break-words leading-relaxed text-sm lg:text-base">
                      {transaction.note}
                    </div>
                  </div>
                )}
                ...
                >
                  <div className="flex items-center gap-3 lg:gap-4">
                    ...
                    <div className="text-left">
                      ...
                      <div className="font-black text-blue-900 text-sm lg:text-base">
                        Discuss this expense
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3 lg:space-y-4">
                      {balanceBefore !== undefined && (
                        <div className="flex items-center gap-3 lg:gap-4 p-4 lg:p-5 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl lg:rounded-3xl border border-gray-200 shadow-lg">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[10px] lg:text-xs text-gray-500 font-semibold uppercase tracking-wide">
                              Wallet Balance Before
                            </div>
                            <div className="font-bold text-gray-900 text-sm lg:text-base tracking-tight tabular-nums">
                      ...
                      {balanceAfter !== undefined && (
                        <div className="flex items-center gap-3 lg:gap-4 p-4 lg:p-5 bg-gradient-to-br from-[#4a6850]/5 to-[#3d5643]/5 rounded-2xl lg:rounded-3xl border border-[#4a6850]/20 shadow-lg">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div
                              className={`font-bold text-sm lg:text-base tracking-tight tabular-nums ${
      ...
      >
        <div
          style={{
            background:
              "linear-gradient(135deg, #4a6850 0%, #3d5643 50%, #2f4a35 100%)",
          ...
          <div style={{ textAlign: "center", marginBottom: "24px" }}>
            <div
              style={{
                ...
                marginBottom: "12px",
                background:
                  transaction.type === "expense" ? "#FEF2F2" : "#F0FDF4",
            ...
            {transaction.fromName && transaction.toName && (
              ...
              >
                ...
                <div>
                  ...
                  >
                    {resolvedFromName} → {resolvedToName}
                  </div>
                  {transaction.method && (
                    <div
                      style={{
                  ...
                  {resolvedParticipants?.map((p: any, i: number) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: "6px 0",
                        borderBottom:
                          i < transaction.participants.length - 1
                            ? "1px solid #F3F4F6"
                            : "none",
                      }}
                    >
