        ...
        {onRetry && (
          <button
            onClick={() => {
              setIsRetrying(true);
              onRetry();
              // In case onRetry doesn't navigate away or reload the page immediately,
              // we reset the visual state after a short delay so the user isn't stuck.
              retryTimerRef.current = setTimeout(() => setIsRetrying(false), 2000);
            }}
            disabled={isRetrying}
            className={`w-full h-12 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center gap-2 ${isRetrying ? 'opacity-80 cursor-not-allowed' : 'active:scale-95'}`}
          >
            {isRetrying ? (
              <>
