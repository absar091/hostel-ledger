              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {invitedEmails.map(email => (
                    <TooltipProvider delayDuration={0} key={email}>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div
                            className="w-full flex items-center text-left gap-3 p-3 bg-purple-50/50 border border-purple-100 rounded-2xl animate-fade-in focus-visible:ring-2 focus-visible:ring-purple-400 focus-visible:outline-none"
                            tabIndex={0}
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <span className="text-[10px] text-purple-600 font-bold flex items-center gap-1">
                                <span className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse shrink-0"></span> {t('sheets.create_group.email_invite')} • {t('sheets.create_group.new_user')}
                              </span>
                            </div>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleRemoveEmailInvite(email);
                              }}
                              className="w-8 h-8 rounded-full bg-red-100/50 hover:bg-red-100 flex items-center justify-center text-red-500 transition-colors z-10 shrink-0 focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:outline-none"
                        ...
                        <TooltipContent side="top" sideOffset={4} className="bg-slate-900 text-white font-medium text-xs px-3 py-1.5 shadow-xl max-w-[200px] text-center">
                  ...
                  {invitedUsernames.map(username => (
                    <TooltipProvider delayDuration={0} key={username}>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div
                            className="w-full flex items-center text-left gap-3 p-3 bg-emerald-50/50 border border-emerald-100 rounded-2xl animate-fade-in focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none"
                            tabIndex={0}
                          >
                            <div className="w-9 h-9 rounded-full bg-emerald-100 flex items-center justify-center text-sm font-bold text-emerald-600 shrink-0">
                              {username.charAt(0).toUpperCase()}
                            </div>
                            <div className="flex-1">
                              <span className="font-black text-sm text-gray-900 block">@{username}</span>
                              <span className="text-[10px] text-emerald-600 font-bold flex items-center gap-1">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shrink-0"></span> {t('sheets.create_group.app_user')} • {t('sheets.create_group.invite_sent')}
                              </span>
                            </div>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleRemoveInvite(username);
                              }}
                              className="w-8 h-8 rounded-full bg-red-100/50 hover:bg-red-100 flex items-center justify-center text-red-500 transition-colors z-10 shrink-0 focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:outline-none"
