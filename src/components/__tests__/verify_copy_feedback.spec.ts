...
test('verify copy id feedback', async ({ page }) => {
  ...
  // Find the Copy ID button (using the aria-label attribute)
  // Note: component was updated from "Copy ID" to "Copy Reference"
  const copyButton = page.locator("button[aria-label='Copy ID'], button[aria-label='Copy Reference']").first();
