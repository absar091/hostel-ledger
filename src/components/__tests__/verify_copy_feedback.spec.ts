...
test('verify copy id feedback', async ({ page }) => {
  ...
  // Find the Copy ID button (using the aria-label attribute which is initially "Copy ID")
  const copyButton = page.locator("button[aria-label='Copy ID']");

  // Click the copy button
  await copyButton.click();

  // Wait for the feedback state (icon change and aria-label update)
