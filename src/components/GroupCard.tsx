  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
          <button
            onClick={onClick}
            aria-label={`Group ${name} with ${memberCount} members. ${balance === 0 ? 'Settled up' : isPositive ? `You will receive Rs ${Math.abs(balance).toLocaleString()}` : `You owe Rs ${Math.abs(balance).toLocaleString()}`}`}
