  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
          <button
            onClick={onClick}
            aria-label={`${name} group with ${memberCount} members. ${isPositive ? `You will receive Rs ${Math.abs(balance).toLocaleString()}` : `You owe Rs ${Math.abs(balance).toLocaleString()}`}`}
            className="w-full bg-card rounded-xl p-4 shadow-card hover:shadow-card-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 transition-all duration-200 flex items-center gap-4 group"
          >
