...
const GroupCard = ({ name, balance, memberCount, emoji = "👥", onClick }: GroupCardProps) => {
  ...
  const ariaLabel = `${name} group with ${memberCount} members. ${isPositive ? "You'll receive" : "You owe"} Rs ${Math.abs(balance).toLocaleString()}.`;
    ...
    <Tooltip>
      <TooltipTrigger asChild>
          <button
            ...
            className="w-full bg-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-xl p-4 shadow-card hover:shadow-card-hover transition-all duration-200 flex items-center gap-4 group"
          >
