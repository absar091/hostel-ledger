  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
          <button
            onClick={onClick}
            aria-label={`${name} group with ${memberCount} members. You ${isPositive ? "will receive" : "owe"} Rs ${Math.abs(balance).toLocaleString()}.`}
            className="w-full bg-card rounded-xl p-4 shadow-card hover:shadow-card-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring transition-all duration-200 flex items-center gap-4 group"
          >
