...
const GroupCard = ({ name, balance, memberCount, emoji = "👥", onClick }: GroupCardProps) => {
  ...
  const ariaLabel = `${name} group with ${memberCount} members. ${isPositive ? `You will receive Rs ${Math.abs(balance).toLocaleString()}` : `You owe Rs ${Math.abs(balance).toLocaleString()}`}.`;
