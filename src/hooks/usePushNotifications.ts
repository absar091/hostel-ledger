...
export const usePushNotifications = () => {
  ...
  const subscribe = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log('🔔 Requesting FCM token from Firebase Messaging SDK...');
      ...
      try {
        ...
        try {
          ...
          if (currentUser?.uid) {
            ...
            console.log('📤 Token data:', { userId: currentUser.uid, tokenLength: fcmToken.length });
        ...
        logger.error("Failed to get FCM token", { error: tokenError.message });
  ...
  const unsubscribe = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      try {
        ...
        if (currentUser?.uid) {
          const apiUrl = import.meta.env.VITE_API_URL || 'https://hostel-ledger-backend.vercel.app';
          await fetch(`${apiUrl}/api/push-unsubscribe/${currentUser.uid}`, {
            method: 'DELETE',
            headers: { 
              'Cache-Control': 'no-cache'
            }
