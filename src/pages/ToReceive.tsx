...
const ToReceive = () => {
      ...
      Object.entries(groupSettlements).forEach(([memberId, settlements]) => {
        if (settlements.toReceive > 0) {
          ...
          people.push({
            ...
            name:
              member?.name ||
              t("common.member_fallback", {
                username: memberId.substring(0, 5),
              }),
  ...
  useEffect(() => {
    peopleWhoOweMe.forEach((person) => {
      ...
      if (
        group &&
        (!group.members || group.members.length === 0 || !memberFound)
      ) {
  ...
  const totalToReceive = peopleWhoOweMe.reduce(
  ...
  const formatPaymentDetails = (
    paymentDetails?: PersonToReceiveFrom["paymentDetails"],
  ) => {
    if (!paymentDetails) return null;

    const details = [];
    if (paymentDetails.jazzCash)
      details.push(`${t("common.jazzcash")}: ${paymentDetails.jazzCash}`);
    if (paymentDetails.easypaisa)
      details.push(`${t("common.easypaisa")}: ${paymentDetails.easypaisa}`);
    if (paymentDetails.bankName && paymentDetails.accountNumber) {
      details.push(
        `${paymentDetails.bankName}: ${paymentDetails.accountNumber}`,
      );
    }
    if (paymentDetails.raastId)
      details.push(`${t("common.raast_id")}: ${paymentDetails.raastId}`);
        ...
        <main className="px-4 pt-6">
          ...
          <div className="bg-gradient-to-br from-[#e8f5e9] to-[#f1f8f4] rounded-3xl p-5 md:p-8 shadow-lg border border-[#4a6850]/10 relative overflow-hidden mb-6">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm text-[#4a6850]/70 font-black tracking-wide uppercase">
                  {t("to_receive.total_amount")}
                </span>
              </div>
              ...
              <div className="text-sm text-[#4a6850] font-bold">
                {t("to_receive.from_count_people", {
                  ...
                  people: t(
                    peopleWhoOweMe.length === 1
                      ? "to_receive.person_singular"
                      : "to_receive.person_plural",
                    { count: peopleWhoOweMe.length },
                  ),
                })}
              </div>
          ...
          {peopleWhoOweMe.length > 0 ? (
                    ...
                    <div className="flex-1 min-w-0 pr-12 md:pr-14">
                      <div className="flex flex-col mb-2 md:mb-3">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-black text-gray-900 text-base md:text-lg truncate tracking-tight">
                            {person.name}
                          </h3>
                          {person.isTemporary && (
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-2 mb-3 md:mb-4">
                        {person.phone && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs text-[#4a6850]/70 font-bold">
                              {person.phone}
                            </span>
                          </div>
                        )}
                      ...
                      <div className="flex items-center gap-2 pt-2 md:pt-3 border-t border-[#4a6850]/10">
                        ...
                        <span className="text-xs text-[#4a6850]/80 font-black uppercase tracking-wider">
                          {person.groupName}
                        </span>
                        <span className="text-[10px] text-[#4a6850]/30 font-bold ml-auto">
                          {t("to_receive.tap_to_view")}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="text-center py-12 bg-white rounded-3xl border border-[#4a6850]/10 shadow-[0_20px_60px_rgba(74,104,80,0.08)]">
              ...
              <h3 className="font-black text-gray-900 mb-1 tracking-tight">
