...
const Activity = () => {
  ...
  const { shouldShowPageGuide, markPageGuideShown } = useUserPreferences(
    user?.uid,
  );
  ...
  const [filterType, setFilterType] = useState<"all" | "expense" | "payment">(
    "all",
  );
        ...
        <div className="px-6 pt-8 pb-6">
          <div className="flex items-center gap-4 mb-6 lg:mb-8">
            <div>
              ...
              <p className="text-xs lg:text-sm text-[#4a6850]/80 font-bold">
          ...
          <div className="grid grid-cols-2 gap-3 lg:gap-4 mb-6 lg:mb-8">
            <div className="bg-white rounded-3xl p-4 lg:p-5 border border-[#4a6850]/10 shadow-[0_20px_60px_rgba(74,104,80,0.08)]">
              <div className="text-[10px] lg:text-xs text-[#4a6850]/70 mb-1.5 lg:mb-2 font-black uppercase tracking-widest">
                {t("group.expenses_count")}
              </div>
              <div className="text-2xl lg:text-3xl font-black text-gray-900 tracking-tight">
                {stats.totalTransactions}
              </div>
            </div>

            <div className="bg-white rounded-3xl p-4 lg:p-5 border border-red-500/10 shadow-[0_20px_60px_rgba(239,68,68,0.08)]">
              <div className="text-[10px] lg:text-xs text-red-500/70 mb-1.5 lg:mb-2 font-black uppercase tracking-widest">
                {t("activity.stats.total_spent")}
              </div>
              <div className="text-2xl lg:text-3xl font-black text-red-600 tracking-tight tabular-nums">
            ...
            <div className="bg-white rounded-3xl p-4 lg:p-5 border border-[#4a6850]/10 shadow-[0_20px_60px_rgba(74,104,80,0.08)]">
              <div className="text-[10px] lg:text-xs text-[#4a6850]/70 mb-1.5 lg:mb-2 font-black uppercase tracking-widest">
                {t("activity.stats.total_received")}
              </div>
              <div className="text-2xl lg:text-3xl font-black text-[#4a6850] tracking-tight tabular-nums">
          ...
          <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide mb-4">
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-sm font-black whitespace-nowrap transition-all shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4a6850] focus-visible:ring-offset-2 ${
                filterType === "all"
          ...
          <div className="flex gap-3 mt-3 overflow-x-auto pb-3 scrollbar-hide">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-2xl text-xs font-black whitespace-nowrap transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4a6850] focus-visible:ring-offset-2 ${
                filterDate === "all"
          ...
          {filteredTransactions.length > 0 ? (
            <div className="space-y-4">
              {filteredTransactions.map((transaction, index) => {
                ...
                const isPayer =
                  transaction.paidBy === user?.uid || transaction.userIsPayer;
                const userParticipant =
                  transaction.participants?.find(
                    (p: any) =>
                      p.id === user?.uid || (p as any).userId === user?.uid,
                  ) ||
                  (transaction.userIsParticipant
                    ? { amount: transaction.userShare || 0 }
                    : null);
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-11 lg:w-12 h-11 lg:h-12 rounded-2xl flex items-center justify-center border shadow-lg group-hover:scale-105 transition-transform ${getTransactionColor(transaction.type)}`}
                      >
                        {getTransactionIcon(transaction.type)}
                      </div>

                      <div className="flex-1 min-w-0">
                        <div className="font-black text-gray-900 text-base lg:text-lg tracking-tight mb-0.5 lg:mb-1 truncate">
                          {transaction.title}
                        </div>
                        ...
                        {transaction.note && (
                          <div className="text-[10px] lg:text-xs text-gray-500 mt-0.5 lg:mt-1 font-medium truncate">
                            {transaction.note}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="text-right flex-shrink-0">
                        <div
                          className={`font-black text-base lg:text-xl tabular-nums tracking-tight ${
                            transaction.type === "expense"
                              ? isPayer || isParticipant
                                ? "text-red-600"
                                : "text-slate-400"
                        ...
                        {transaction.method && (
                          <div className="text-[10px] lg:text-xs text-gray-500 mt-0.5 lg:mt-1 font-bold capitalize">
                            {transaction.method}
                          </div>
                        )}
                      </div>
            ...
            <div className="text-center py-16 bg-white rounded-3xl border border-[#4a6850]/10 shadow-[0_20px_60px_rgba(74,104,80,0.08)]">
              ...
              <h3 className="text-lg lg:text-xl font-black text-gray-900 mb-2 lg:mb-3 tracking-tight">
