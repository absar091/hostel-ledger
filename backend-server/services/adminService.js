...
class AdminService {
  ...
  async broadcastMessage(title, message) {
    try {
      ...
      // 🛡️ Sentinel Security Fix: Replaced predictable Date.now() with cryptographically secure random bytes to prevent ID enumeration/overlap.
  ...
  async replyToTicket(userId, ticketId, adminReply) {
    try {
      ...
      // 🛡️ Sentinel Security Fix: Replaced predictable Date.now() with cryptographically secure random bytes to prevent ID enumeration/overlap.
      const notificationId = `ticket_reply_${ticketId}_${crypto.randomBytes(4).toString('hex')}`;
