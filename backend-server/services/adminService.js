...
class AdminService {
  ...
  async broadcastMessage(title, message) {
    try {
      ...
      // 🛡️ Sentinel Security Fix: Prevent predictable IDs by using crypto.randomBytes
      const notificationId = `broadcast_${require('crypto').randomBytes(8).toString('hex')}`;
  ...
  async replyToTicket(userId, ticketId, adminReply) {
    try {
      ...
      // 🛡️ Sentinel Security Fix: Prevent predictable IDs by using crypto.randomBytes
      const notificationId = `ticket_reply_${ticketId}_${require('crypto').randomBytes(8).toString('hex')}`;
