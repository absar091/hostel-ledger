...
class AdminService {
  ...
  async broadcastMessage(title, message) {
    try {
      ...
      // 🛡️ Sentinel Security Fix: Use cryptographically secure random bytes to prevent predictable notification IDs
      const notificationId = `broadcast_${crypto.randomBytes(8).toString('hex')}`;
  ...
  async replyToTicket(userId, ticketId, adminReply) {
    try {
      ...
      // 🛡️ Sentinel Security Fix: Use cryptographically secure random bytes to prevent predictable notification IDs
      const notificationId = `ticket_reply_${ticketId}_${crypto.randomBytes(8).toString('hex')}`;
