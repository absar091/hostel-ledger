...
app.get('/api/ai/insights', generalLimiter, authenticate, async (req, res) => {
  ...
  try {
    ...
    if (userGroupsSnap.exists()) {
      ...
      // ⚡ Bolt Optimization: Parallelize independent database reads for faster AI Insights response
      // Expected impact: Reduces latency from O(N) to O(1) network roundtrips for group lookups
