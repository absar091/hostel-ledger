...
app.get('/api/budgets/group/:groupId', generalLimiter, authenticate, async (req, res) => {
  try {
    ...
    if (!isValidFirebaseId(groupId)) return res.status(400).json({ success: false, error: 'Invalid group ID format' });
...
app.get('/api/budgets/personal/:userId', generalLimiter, authenticate, async (req, res) => {
  try {
    ...
    if (!isValidFirebaseId(userId)) return res.status(400).json({ success: false, error: 'Invalid user ID format' });
