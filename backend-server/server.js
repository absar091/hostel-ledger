...
app.get('/api/ai/insights', generalLimiter, authenticate, async (req, res) => {
  ...
  try {
    ...
    if (userGroupsSnap.exists()) {
      ...
      // ⚡ Bolt Optimization: Parallelize group name fetching to eliminate N+1 latency.
      // Expected impact: Resolves linear N+1 query bottleneck for users with multiple groups.
